...
def run_orchestrator(query: str, metadata: dict, netcdf_path: str, scenario_path: str = None) -> dict:
    ...
    # === CHANGE STARTS HERE ===
    # We NO LONGER auto-save to memory here.
    # Instead, we extract the code and thought to pass them to the UI.
    generated_code = exec_result.get("code_generated", "")
    plan_thought = plan.get("thought", "")
    
    if exec_result["success"]:
        steps_log.append({"stage": "Execution Success", "status": "complete", "output": "Code generated. Waiting for user approval."})
    # ==========================
    ...
    return {
        ...
        "steps_log": steps_log,
        # NEW: Return these to the frontend
